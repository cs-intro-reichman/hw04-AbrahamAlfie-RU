...
public class ArrCharOps {
    ...
    public static boolean equals(char[] arr1, char[] arr2) {
        ...
        for (int i = 0; i < arr1.length; i++) {
            newStr = (arr1[i] == arr2[i]) ? newStr += arr1[i] : newStr;
        }

        return (newStr.length() == arr1.length);
    }
    ...
    public static char[] concat(char[] arr1, char[] arr2) {
        ...
        for (int i = 0, j = arr1.length; (i < arr2.length) || (j > newArr.length); i++, j++) {
            newArr[j] = arr2[i];
        }
        return newArr;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        ...
        for (int i = 0, j = beginIndex; j < endIndex; i++, j++) {
            newArr[i] = arr[j];
        }
        return newArr;
    }
    ...
    public static int compareTo(String str1, String str2) {
        int result = 0;
        // Returns error code when either string is empty
        if (str1.length() == 0 || str2.length() == 0) {
            return -2;
        }
        // Returns 0 if the strings are equal, 1 if not
        if (str1.length() == str2.length()) {
            for (int i = 0; i < str1.length(); i++) {
                result = str1.charAt(i) == str2.charAt(i) ? 0 : 1;
                if (result == 1) {
                    break;
                }
            }
            ...
            for (int i = 0; i < Math.min(str1.length(), str2.length()); i++) {
                if (str1.charAt(i) != str2.charAt(i)) {
                    result = str1.charAt(i) < str2.charAt(i) ? -1 : 1;
                } else {
